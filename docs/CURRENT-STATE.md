...
- **F1.7** — SE1-T93 (Actor/Roles, RFC-007). Tablas `actores` (supertipo persona XOR entidad), `catalogo_roles_actor` (13 roles seed), `actor_roles` (asignación declarativa con vigencia + scope). Backfill 1:1 → **2.743 actores** (2.739 persona + 4 entidad = personas/entidades vivas). RLS multi-tenant + soft-delete, advisor limpio, typecheck verde, aplicado a prod y verificado. Mergeado y taggeado por decisión de Yair (sin UI → no requiere smoke visual).
  ...
  - **Roles migrados a `actor_roles` (prod, paridad OK):** `socio`=2.348 (socio_padron 2347 + socio 1), `jugador`=165, `proveedor`=1. Vista `v_actores_roles` = 2.514 filas. Trackeado en **SE1-T95**.
  - **Pendiente incremental (RFC-007 §9):** `suscriptor` (51, requiere alta en `catalogo_roles_actor`), enriquecer scope (jugador→equipo/disciplina desde `personas_equipos`; socio→padrón), swap de lectura en pages/módulos (con smoke), clasificar atributos ambiguos (staff/dirigente/etc.), deprecar role-atributos migrados (coexistencia mientras tanto, ADR-068).

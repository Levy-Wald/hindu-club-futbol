...
| Portal buildout + bug fix (24-jun) | Tras el smoke de Yair (portal flaco + bug). **Bug fix:** aceptar invitaciones fallaba (CHECK femenino vs código masculino) → alineado, ahora funciona. **Secciones nuevas del portal:** Mi equipo+partidos, Reservar espacio, Detalle de evento (clickeable, aceptar inline), Descargar resumen de pagos (CSV), Mensajes (→notificaciones), El club (sedes+Maps). SE1-T48 → 75%. Pendiente: edición ficha completa, torneos/posiciones. |
...
| Loop paralelo al CUIT (24-jun) | 3 tracks DONE: **(1)** dedup de 2 duplicados de menú (competencias/rrhh_basico). **(2)** Tier 3 hardening — `search_path` pineado en ~33 funciones propias (SE1-I8, cierra el lint `function_search_path_mutable`). **(3)** Pre-cableado F5: seams de los **4 conectores** listos (mock-first, ADR-035) — **email** (Resend, `email-adapter.ts`), **pagos** (MercadoPago, `lib/connectors/payments/` — botón Pagar ya lo consume), **mensajería** (WhatsApp, `lib/connectors/messaging/`) y **calendario** (Google/Outlook/CalDAV, `lib/connectors/calendar/`). F5 = implementar el adapter real + flip de env, sin reconstruir. **(4)** Tests: lógica pura de Compras + slugify (16 unit) + E2E happy-path (proveedores/compras/portal) + fix infra vitest (alias `@`, suite 200/200). |
| Fase activa | **F3 — Portal Cliente** (front del socio mobile). F1 (Troncal) y F2 (Vertical CCBP) cerrados. **SE1-T48 → 95%** (núcleo + edición contacto; sólo resta pago real F5 + DONE visual). |
| F3.1 Portal Cliente (23-jun) | **Núcleo completo, DONE técnico.** Route group `/portal/[tenant]` paralelo a `/admin`. 6 sub-sprints: (1) Fundación v0.46.0 (middleware+shell mobile+dashboard), (2) Mi cuenta v0.46.1 (cuotas+saldo+historial+pago mock→F5), (3) Mi perfil+familia v0.46.2 (read-only), (4) Notificaciones v0.46.3 (campana+contador), (5) Login-branching v0.46.3 (admin→/admin, socio→/portal en form+callback+middleware), (6) Mi agenda v0.46.4 (eventos+aceptar/rechazar invitaciones, reúsa F1.4), (7) cierre fino v0.46.5 (edición de datos de contacto del socio, self-scoped). **Pendiente (5%):** pago real (F5, bloqueado por CUIT), DONE visual. |
...
| Tier 3 hardening seguridad (**I-008**) | Resto del advisor tras la pasada F1 (ERRORs en 0). **HECHO (24-jun):** `function_search_path_mutable` → pineado `search_path=pg_catalog,public` en las ~33 funciones propias (excluye extensiones). **Queda:** 82 `*_security_definer_function_executable` (revisar caso por caso — muchas son RPCs intencionales del cliente), 3 `extension_in_public` (riesgoso/breaking), 1 `public_bucket_allows_listing` (**Yair**: ¿intencional?), 1 `auth_leaked_password_protection` (**Yair**: toggle en dashboard Auth). Los 2 `rls_policy_always_true` (`api_logs`, `pre_inscripciones_insert_anon`) son INSERT anónimos by-design → no son holes. | Parcial. Lo que queda: no bloqueante (single-tenant pre-F4), 2 ítems de la mano de Yair. |

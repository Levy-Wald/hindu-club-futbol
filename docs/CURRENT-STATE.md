...
| Indicador | Valor |
|---|---|
| Tag git actual | `v0.46.2-portal-perfil` (F3.1c). Previo: `v0.46.1-portal-mi-cuenta`, `v0.46.0-portal-fundacion`, serie cierre-fino-F1 v0.45.x, `v0.44.0-proveedores-ui`, serie seguridad v0.43.x |
| Fase activa | **F3 — Portal Cliente** (front del socio mobile). F1 (Troncal) y F2 (Vertical CCBP) cerrados. **SE1-T48 → 55%** (3 de ~6 sub-sprints). |
| F3.1 Portal Cliente (23-jun) | **DONE técnico sub-sprints 1-3.** (1) **Fundación** v0.46.0: route group `/portal/[tenant]` paralelo a `/admin`, middleware, shell mobile bottom-nav, dashboard (membresía+saldo+próxima cuota). (2) **Mi cuenta** v0.46.1: cuotas+estado, historial, saldo, pago mock (cobro real=F5). (3) **Mi perfil+familia** v0.46.2: identidad+contacto+vínculos, read-only. **Pendiente:** (4) Agenda (eventos+inscripción), (5) Notificaciones, (6) login-branching socio↔admin + edición perfil. |

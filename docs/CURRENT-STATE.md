...
| Loop paralelo al CUIT (24-jun) | 3 tracks DONE: **(1)** dedup de 2 duplicados de menú (competencias/rrhh_basico). **(2)** Tier 3 hardening — `search_path` pineado en ~33 funciones propias (SE1-I8, cierra el lint `function_search_path_mutable`). **(3)** Pre-cableado F5: adapters de **email** (Resend, mock-first) y **pagos** (MercadoPago) listos en `lib/connectors/payments/` + `email-adapter.ts` — F5 = implementar real + flip de env, sin reconstruir. |

...
import { ejecutarCuotaVencida7d } from '@/modules/comunicaciones/lib/triggers/cuota-vencida-7d'
...
export async function GET(request: NextRequest) {
  ...
  await supabase.from('com_jobs_log').insert({
    id: jobId,
    tenant_id: TENANT_ID,
    job_slug: 'cuota_vencida_7d',
    status: 'running',
  })

  try {
    const result = await ejecutarCuotaVencida7d(supabase, TENANT_ID, jobId)

...
export function EquiposFilters({ disciplinas }: EquiposFiltersProps) {
  ...
  const [disciplina, setDisciplina] = useState(currentDisciplina)
  const [activo, setActivo] = useState(currentActivo)

  const activeCount = (disciplina ? 1 : 0) + (activo ? 1 : 0)
  ...
  }, [disciplina, activo, searchParams, router])
      ...
      <Popover>
        ...
        <PopoverContent className="w-72" align="start">
          <div className="space-y-4">
            <div>
              <h4 className="font-medium text-sm mb-2">Disciplina</h4>
              <div className="space-y-1.5">
                {disciplinas.map((d) => (
                  <label key={d} className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox
                      checked={disciplina === d}
                      onCheckedChange={() => setDisciplina(disciplina === d ? '' : d)}
                    />
                    {d}
                  </label>
                ))}

...
import { disconnectIntegracion, triggerManualSyncMicrosoft } from '@/lib/calendar-sync/actions'
...
export function MicrosoftCalendarCard({ personaId, integracion, microsoftAuthUrl }: MicrosoftCalendarCardProps) {
  ...
  function handleDisconnect() {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        setMessage('Microsoft Calendar desconectado')
        router.refresh()
      } else {
  ...
  function handleSync() {
    startTransition(async () => {
      setMessage(null)
      const result = await triggerManualSyncMicrosoft(personaId)
      if (result.ok) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Sincroniza tus eventos del club con tu calendario de Outlook.
        </CardDescription>
      </CardHeader>
        ...
        {integracion && config ? (
          <>
            ...
            <div className="flex gap-2">
              ...
              {(integracion.estado === 'error' || integracion.estado === 'expired') && (
                <Button
                  size="sm"
                  render={<a href={microsoftAuthUrl} />}
                >
                  Reconectar
                </Button>
              )}
          ...
          <div className="text-center py-4">
            <p className="text-sm text-muted-foreground mb-4">
              Conecta tu cuenta de Microsoft para sincronizar eventos con Outlook.
            </p>
            <Button
              render={<a href={microsoftAuthUrl} />}
            >

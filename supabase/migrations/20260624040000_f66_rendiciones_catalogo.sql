-- F6.6 — Rendición de gastos: registrar módulo en el catálogo + activar tenant.
...
VALUES (
  'rendiciones_gastos', 'Rendición de gastos', 'Rendiciones',
  'Rendición de gastos: borrador → presentada → aprobada → liquidada',
  'troncal', 'cross_vertical',
  '/admin/finanzas/rendiciones', 'ReceiptText', 'finanzas', 'Gastos', 10,
  'finanzas.write', 'back_office', true, true
)
...
VALUES ('11111111-1111-1111-1111-111111111111', 'rendiciones_gastos', true, now())
ON CONFLICT (tenant_id, modulo_slug) DO UPDATE SET activo = true, fecha_desactivacion = NULL;

-- ROLLBACK:
--   DELETE FROM tenant_modulos WHERE tenant_id='11111111-1111-1111-1111-111111111111' AND modulo_slug='rendiciones_gastos';
--   DELETE FROM catalogo_modulos WHERE slug='rendiciones_gastos';

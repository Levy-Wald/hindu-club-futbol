...
} from '@/modules/concesiones/lib/actions'
  ...
  return (
    <>
      ...
      {puntosVenta.length === 0 ? (
        <p className="text-muted-foreground text-sm">No hay puntos de venta. Crea uno para empezar.</p>
      ) : (
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div><Label>Min.</Label><Input type="number" min={0} value={form.stock_minimo} onChange={e => setForm(f => ({ ...f, stock_minimo: Number(e.target.value) }))} /></div>
              </div>
              <Button className="w-full" disabled={isPending} onClick={() => {
                startTransition(async () => {
              ...
              {activos.map((p: any) => (
                <tr key={p.id} className="border-b last:border-0">
                  ...
                  <td className="py-2 pr-4 text-right">
                    ...
                    {p.stock_minimo > 0 && <span className="text-muted-foreground text-xs"> (min: {p.stock_minimo})</span>}
                  </td>
                  <td className="py-2">
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div><Label>Motivo</Label><Input value={motivo} onChange={e => setMotivo(e.target.value)} placeholder="Reposicion, merma, etc." /></div>
            ...
            <tbody>
              {ventas.map((v: any) => {
                const comprador = v.comprador
                  ...
                  : (v.comprador_nombre_libre || 'Anonimo')
                return (
                      ...
                      {v.estado !== 'anulada' && (
                        <Button
                          ...
                          onClick={() => {
                            ...
                            startTransition(async () => {
                              await anularVenta(v.id, 'Anulacion manual')
                            })
                          }}
                        >
...
function CanonTab({ concesionarioId, canones }: { concesionarioId: string; canones: any[] }) {
  ...
  return (
    <>
      ...
      {canones.length === 0 ? (
        <p className="text-muted-foreground text-sm mt-4">No hay canones calculados.</p>
      ) : (
...
function MPTab({ concesionario }: { concesionario: any }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Configuracion MercadoPago</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          {concesionario.mp_modo === 'mock'
            ? 'Los links de pago son simulados. Configura credenciales reales para activar pagos.'
            : concesionario.mp_modo === 'sandbox'
            ? 'Modo sandbox: los pagos se procesan en entorno de prueba.'
            : 'Produccion: los pagos son reales.'}
        </p>

...
export async function ajustarStock(id: string, cantidadDelta: number, motivo: string) {
  ...
  // Notificar si stock bajo minimo
  if (nuevoStock <= (prod.stock_minimo ?? 0) && prod.stock_minimo && prod.stock_minimo > 0) {
    ...
    if (conc?.persona_id) {
      crearNotificacion({
        ...
        mensaje: `El producto "${prod.nombre}" tiene stock ${nuevoStock} (minimo: ${prod.stock_minimo}).`,
        prioridad: 'media',
...
export async function anularVenta(ventaId: string, motivo: string) {
  ...
  if (venta.estado === 'anulada') return formatResult(false, 'La venta ya esta anulada')
  ...
  for (const item of items ?? []) {
    await ajustarStock(item.producto_id, item.cantidad, 'Anulacion de venta')
  }
...
export async function calcularCanonPeriodo(concesionarioId: string, periodo: string) {
  ...
  crearNotificacion({
    ...
    mensaje: `Se calculo el canon del periodo ${periodo} para un concesionario.`,
    prioridad: 'media',

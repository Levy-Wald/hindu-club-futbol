...
test.describe('Comunicaciones', () => {
  ...
  test('crear plantilla nueva', async ({ page }) => {
    ...
    // Should now have 19 rows (18 seed + 1 new)
    await expect(page.locator('[data-testid="plantillas-row"]')).toHaveCount(19, { timeout: 10000 })
  })
  ...
  test('soft-delete plantilla user-created', async ({ page }) => {
    ...
    // Find and edit the new plantilla
    await page.getByText(`Borrable ${ts}`).click()
